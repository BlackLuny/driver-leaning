...
{  
	...
	{   //hook
		...
		__asm //去掉页面保护
		{
				cli
				mov eax,cr0
				and eax,not 10000h //and eax,0FFFEFFFFh 第16位是写保护 当为1时禁止写 当为0时可以写
				//not 10000h 即取反 0FFFEFFFFh
				mov cr0,eax
...
{   //
	...
	//对相应的IPR进行处理
	pIrp->IoStatus.Information=info;			//设置操作的字节数为4 
...
/*
使用IoGetCurrentIrpStackLocation得到调用者堆栈区域指针IrpStack(PIO_STACK_LOCATION类型)
...
直接方式:
MmGetSystemAddressForMdlSafe映射IRP->MdlAddress地址到内核空间
MmGetMdlByteCount,得到MDL大小,以字节为单位.通常应该在IRQL<=DISPATCH_LEVEL的情况下使用MDL,那么如何输出呢?和Buffered方式一样从一个地方读,处理完再写到同一个地方吗?(问题1)
